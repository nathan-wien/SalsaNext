...
def one_hot_pred_from_label(y_pred, labels):
    ...
    y_true[torch.arange(labels.size(0)), indexes] = ones[
        torch.arange(labels.size(0)), indexes
    ]
...
class SoftmaxHeteroscedasticLoss(torch.nn.Module):
    ...
    def forward(self, outputs, targets, eps=1e-5):
        ...
        targets = (
            torch.nn.functional.one_hot(targets, num_classes=20)
            .permute(0, 3, 1, 2)
            .float()
        )
        ...
        precision = 1 / (var + eps)
        return torch.mean(
            0.5 * precision * (targets - mean) ** 2 + 0.5 * torch.log(var + eps)
        )
...
def save_checkpoint(to_save, logdir, suffix=""):
    # Save the weights
    torch.save(to_save, logdir + "/SalsaNext" + suffix)
...
class Trainer:
    def __init__(self, ARCH, DATA, datadir, logdir, path=None, uncertainty=False):
        ...
        if self.path is not None:
            ...
            w_dict = torch.load(
                path + "/SalsaNext", map_location=lambda storage, loc: storage
            )
    ...
    def train_epoch(
        ...
        for (
            ...
            if self.uncertainty:
                ...
                loss_m = (
                    criterion(output_mean.clamp(min=1e-8), proj_labels)
                    + hetero
                    + self.ls(output_mean, proj_labels.long())
                )
        ...
        with torch.no_grad():
            ...
            for (
                ...
                if self.uncertainty:
                    ...
                    hetero = self.SoftmaxHeteroscedasticLoss(
                        mean.float(), proj_labels.float()
                    ).mean()
