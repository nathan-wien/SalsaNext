...
class User:
    def __init__(
        self, ARCH, DATA, datadir, logdir, modeldir, split, uncertainty, mc=30
    ):
        # parameters
        self.ARCH = ARCH
        self.DATA = DATA
        ...
        with torch.no_grad():
            ...
            if self.uncertainty:
                ...
                w_dict = torch.load(
                    modeldir + "/SalsaNext", map_location=lambda storage, loc: storage
                )
        ...
        with torch.no_grad():
            ...
            for (
                ...
                if self.uncertainty:
                    ...
                    res = time.time() - end
                    print(
                        "KNN Infered seq", path_seq, "scan", path_name, "in", res, "sec"
                    )
                    knn.append(res)
                    end = time.time()
